...
class QuillImageUpload {
	...
	selectLocalImage() {
		...
		input.onchange = () => {
			...
			$mx.request({url: '/pictures/upload', method: 'post', data: formData/*
, onUploadProgress: (pe) => {
				console.log(pe);
			}
			*/}).then((r) => {
...
const Parchment = Quill.import('parchment');
	...
	//, 'color',
_.each(['background', 'font', 'align'], s => {
	let style = Quill.import('attributors/style/'+s);
	style.scope = Parchment.Scope.ANY;
	Quill.register(style);
})

let Size = Quill.import('attributors/style/size');
Size.scope = Parchment.Scope.ANY;
Size.whitelist = null;
Quill.register(Size);
...
_.each(['border-bottom', 'border-top', 'border-left', 'border-right', 'border', 'letter-spacing', 'border-radius', 'width', 'text-decoration', 'display'], s => {
	const p = new Parchment.Attributor.Style(s, s, {
...
_.each(['padding', 'margin'], s => {
	const p = new Parchment.Attributor.Style(s, s, {
	    scope: Parchment.Scope.BLOCK
	});
	
	p.whitelist = null;//["15px", "15px 0px", "0px 15px", "15px 15px 0px", "0px"];
	Quill.register(p);
});

 var boxAttributor = new Parchment.Attributor.Class('class', 'button', {
...
// TableCell.allowedChildren = [Link];
...
let Block1 = Quill.import('blots/block');

TableCell.blotName = 'td';
TableCell.tagName = 'td';
TableCell.scope = Parchment.Scope.BLOCK_BLOT;
TableCell.defaultChild = 'block';
TableCell.allowedChildren = [Block1, Link, ContainBlot];
Quill.register(TableCell);
*/
/*
